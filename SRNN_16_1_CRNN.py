...
from keras.layers import Input, Embedding, GRU, Bidirectional, TimeDistributed, Dense, Conv1D, MaxPooling1D
import codecs
...
for i in range(x_test_padded_seqs.shape[0]):
    split1=np.split(x_test_padded_seqs[i],16)
    x_test_padded_seqs_split.append(split1)

x_val_padded_seqs_split=[]
for i in range(x_val_padded_seqs.shape[0]):
    split1=np.split(x_val_padded_seqs[i],16)
    x_val_padded_seqs_split.append(split1)
   
x_train_padded_seqs_split=[]
for i in range(x_train_padded_seqs.shape[0]):
    split1=np.split(x_train_padded_seqs[i],16)
    x_train_padded_seqs_split.append(split1)
...
#build model
print("Build Model")
input1 = Input(shape=(int(MAX_LEN/16),), dtype='int32')
...
maxConv1 = MaxPooling1D(pool_length=2)(conv1)
gru1 = Bidirectional(GRU(NUM_FILTERS,recurrent_activation='sigmoid',activation=None,return_sequences=False))(maxConv1)
Encoder1 = Model(input1, gru1)

input2 = Input(shape=(16,int(MAX_LEN/16)), dtype='int32')
...
maxConv2 = MaxPooling1D(pool_length=2)(conv2)
gru2 = Bidirectional(GRU(NUM_FILTERS,recurrent_activation='sigmoid',activation=None,return_sequences=False))(maxConv2)
preds = Dense(5, activation='softmax')(gru2)
